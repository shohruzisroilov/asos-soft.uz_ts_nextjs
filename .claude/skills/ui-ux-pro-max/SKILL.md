...
Comprehensive design guide for web, mobile, and desktop applications. Contains 84 styles, 192 color palettes, 74 font pairings, 192 product types, 98 UX guidelines, 104 icon entries, 16 GSAP motion presets, and 25 chart types across 22 technology stacks. Searchable database with priority-based recommendations.
...
- Choosing color schemes, type systems, spacing scales, or layout systems
...
- The UI looks "unprofessional" but the reason is unclear
- You have received feedback about usability or experience
- You are polishing UI quality before launch
- You need to align design across platforms (Web / iOS / Android)
- You are building a design system or reusable component library

### Skip

This Skill is not needed when:

- Doing pure backend logic development
- Working only on API or database design
- Doing performance work unrelated to the interface
- Doing infrastructure or DevOps work
...
*For human/AI reference: follow priority 1→10 to decide which category to focus on first; use `--domain <Domain>` to query full details. Scripts do not read this table.*
...
| **Choose style / color / font** | "What style fits a fintech app?", "Recommend a color scheme" | Step 2 (design system) |
...
- The default icon library is **Phosphor (`@phosphor-icons/react`)**. The entries listed in `.claude/skills/ui-ux-pro-max/data/icons.csv` are only commonly recommended icons, not the complete set.
- When no suitable icon is found in the recommendation table:
  - **First, keep looking within Phosphor's full icon set for any icon that fits the meaning better**;
  - If Phosphor has no good option either, use **Heroicons (`@heroicons/react`)** as a fallback, keeping the style consistent (outline/filled, stroke weight, corner rounding).
